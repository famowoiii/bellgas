...
Route::middleware(['jwt.auth'])->group(function () {
    ...
    Route::prefix('orders')->group(function () {
        ...
        Route::post('/{order}/status', [OrderStatusController::class, 'updateStatus']);
        Route::get('/{order}/status/available', [OrderStatusController::class, 'getAvailableStatuses']);
        Route::get('/{order}/status/info', [OrderStatusController::class, 'getStatusInfo']);
    });
    ...
    Route::post('test/broadcast', function () {
        ...
        // Create a test order status update event
        $order = \App\Models\Order::where('user_id', $user->id)->first();

        if ($order) {
            event(new \App\Events\OrderStatusUpdated($order, $order->status, 'PROCESSED'));
